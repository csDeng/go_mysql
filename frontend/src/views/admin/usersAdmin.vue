        ...
        <a-popover id='popover'  v-model="visible" :title="'当前编辑用户:' + ( cache ? cache.username : '编辑')" trigger="click">
            <template slot='content'>
                <a-space>
                    ...
                    <a-select :default-value="cache? cache.level: '默认权限'" style="width: 120px" @change="select">
...
export default {
  ...
  methods: {
    ...
    edit(record) {
        // 深拷贝进行数据隔离
        this.cache = {...record};
        const levelMsk = ['普通用户','管理员']
        this.cache.level = levelMsk[this.cache.level] 
        // console.log('edit cache=>',this.cache)
